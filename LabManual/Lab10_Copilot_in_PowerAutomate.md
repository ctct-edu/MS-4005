# Lab10：Microsoft 365 Copilotを使ってPower Appsでアプリを作成する 
...
2. 画面中央の入力フォームに以下のプロンプトを入力して、右下の紙飛行機アイコンをクリックして生成を開始します。
   ...
   ```prompt
   OneDriveに保存されているファイルリストを取得して、整理を行うために定期的なリマインドをTeamsで送信するフローを作成する必要があります。
   ```
...
3. フローと必要なアクションが自動的に作成されます。パラメーターの設定が必要となるため、内容を確認して設定します。
